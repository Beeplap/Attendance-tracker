...
export default function Sidebar({ open, onOpenChange, collapsed = false, onToggleCollapsed }) {
      ...
      <div className={`rounded-2xl border border-purple-200 dark:border-purple-800 bg-white/70 dark:bg-gray-900/80 ${collapsed ? "p-2" : "p-3 sm:p-4"} shadow-sm h-full flex flex-col`}> 
        ...
        <div className={`flex items-center ${collapsed ? "justify-center" : "justify-between"}`}>
          {onToggleCollapsed && (
            ...
            >
              <svg className="w-4 h-4" viewBox="0 0 20 20" fill="currentColor"><path d="M3 6h14M3 10h14M3 14h14" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>
            </button>
          )}
        </div>
        ...
        <nav className="mt-3 space-y-1 flex-1">
          {navItems.map((item, idx) => {
            ...
            const baseItem = "group w-full flex items-center px-3 py-2 rounded-xl border transition-all duration-200 text-sm";
        ...
        <div className={`${collapsed ? "flex justify-center" : ""} mt-3`}>
          <div className={`flex items-center gap-3 p-2 rounded-xl bg-purple-50/70 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 ${collapsed ? "" : "w-full"}`}>
            <img
            ...
            {!collapsed && (
              <div className="min-w-0">
                <div className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">Diana Mary</div>
                <div className="text-[11px] text-gray-600 dark:text-gray-400">Manager</div>
              </div>
            )}
          </div>
        </div>
  ...
  return (
    <>
      ...
      <aside className={`${collapsed ? "w-16" : "w-full sm:w-64 md:w-72 lg:w-72"} shrink-0 hidden sm:block transition-all duration-200 overflow-hidden`}>{Content}</aside>
